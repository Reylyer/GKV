...
float angle = 0., deltaAngle = 0., ratio;
...
void KotakKayu() {
    ...
    glTranslatef(0, 0, 3);
    glBegin(GL_QUADS);
    glTexCoord2f(0., 0.); // kiri bawah
    glVertex3f(-3., -3., 0.);
    glTexCoord2f(1., 0.); // kanan bawah
    glVertex3f(3., -3., 0.);
    glTexCoord2f(1., 1.); // kanan atas
    glVertex3f(3., 3., 0.);
    glTexCoord2f(0., 1.); // kanan bawah
    glVertex3f(-3., 3., 0.);
    glEnd();
    glPopMatrix();
    // bawah
...
void pressKey(int key, int xn, int yn) {
...
void releaseKey(int key, int xn, int yn) {
